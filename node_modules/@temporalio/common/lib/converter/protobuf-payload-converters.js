...
const protojson = __importStar(require("protobufjs/ext/protojson"));
const protobufjslight = __importStar(require("protobufjs/light"));
...
class ProtobufJsonPayloadConverter extends ProtobufPayloadConverter {
    ...
    fromPayload(content) {
        const { messageType, data } = this.validatePayload(content);
        const res = protojson.fromJson(messageType, JSON.parse((0, encoding_1.decode)(data)), {
...
function replaceBuffersInChildren(obj) {
    ...
    if (Array.isArray(obj)) {
        for (let i = 0; i < obj.length; i++) {
            const child = obj[i];
            if (isBuffer(child))
                obj[i] = new Uint8Array(child);
            else
                replaceBuffersInChildren(child);
        }
    }
    else {
        const record = obj;
        for (const [key, child] of Object.entries(record)) {
            if (isBuffer(child))
                record[key] = new Uint8Array(child);
            else
                replaceBuffersInChildren(child);
        }
    }
...
function isBuffer(value) {
    ...
    if (!(0, type_helpers_1.isRecord)(value))
        return false;
    ...
    const maybeBufferConstructor = value.constructor;
    return (maybeBufferConstructor?.name === 'Buffer' &&
...
function getNamespacedTypeName(type) {
