...
exports.extractNexusOperationAlreadyStartedRunId = extractNexusOperationAlreadyStartedRunId;
exports.trimGrpcTypeUrl = trimGrpcTypeUrl;
...
function workflowStatusCodeToNameInternal(code) {
    switch (code) {
        ...
        case proto_1.temporal.api.enums.v1.WorkflowExecutionStatus.WORKFLOW_EXECUTION_STATUS_PAUSED:
            return 'PAUSED';
    }
}
async function executionInfoFromRaw(raw, dataConverter, rawDataToEmbed, namespace) {
...
function rethrowKnownErrorTypes(err) {
    ...
    for (const entry of getGrpcStatusDetails(err) ?? []) {
        if (!entry.type_url || !entry.value)
            continue;
        const type = trimGrpcTypeUrl(entry.type_url);
        switch (type) {
            ...
            case 'temporal.api.errordetails.v1.MultiOperationExecutionFailure': {
                ...
                for (const status of statuses) {
                    ...
                    const statusType = trimGrpcTypeUrl(detail?.type_url);
 ...
 */
function extractNexusOperationAlreadyStartedRunId(err) {
    try {
        for (const entry of getGrpcStatusDetails(err) ?? []) {
            if (!entry.type_url || !entry.value)
                continue;
            const type = entry.type_url.replace(/^type.googleapis.com\//, '');
            if (type !== 'temporal.api.errordetails.v1.NexusOperationExecutionAlreadyStartedFailure')
                continue;
            const details = proto_1.temporal.api.errordetails.v1.NexusOperationExecutionAlreadyStartedFailure.decode(entry.value);
...
function trimGrpcTypeUrl(type_url) {
