...
function appendDefaultInterceptors(interceptors, logger) {
    if (!logger || logger === runtime_1.Runtime.instance().logger)
        return interceptors;
    return {
        activityInbound: [(ctx) => new activity_log_interceptor_1.ActivityInboundLogInterceptor(ctx, logger), ...(interceptors.activityInbound ?? [])],
        activity: interceptors.activity,
        workflowModules: interceptors.workflowModules,
    };
}
function compileWorkerInterceptors({ client, activity, activityInbound, nexus, workflowModules, }) {
    return {
        client: {
...
function compileWorkerOptions(rawOpts, logger, metricMeter) {
    ...
    if (opts.maxEagerActivityReservationsPerWorkflowTask !== undefined &&
...
function nexusServiceHandlersFromOptions(opts) {
...
function toNativeWorkerOptions(opts) {
    ...
    const workflowFailureErrors = [];
    const workflowTypesToFailureErrors = {};
