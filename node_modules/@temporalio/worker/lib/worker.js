...
const continue_as_new_1 = require("@temporalio/common/lib/continue-as-new");
...
const bun_1 = require("./workflow/bun");
...
const patch_activation_callback_1 = require("./workflow/patch-activation-callback");
const errors_1 = require("./errors");
const nexus_1 = require("./nexus");
const conversions_1 = require("./nexus/conversions");
class WorkflowDisposeError extends errors_1.UnexpectedError {
}
...
class Worker {
    ...
    static async create(options) {
        ...
        if (bun_1.isBunPre1_4) {
            logger.warn('Bun versions older than 1.4.0 are deprecated and will not be supported in a future release. Please upgrade to Bun 1.4.0 or later.');
        }
    ...
    static async createWorkflowCreator(workflowBundle, compiledOptions, logger) {
        ...
        const patchActivationCallback = compiledOptions.patchActivationCallback === undefined
            ? undefined
            : (info, patchId) => (0, patch_activation_callback_1.invokePatchActivationCallback)(compiledOptions.patchActivationCallback, info, patchId);
        ...
        if (compiledOptions.debugMode) {
            if (compiledOptions.reuseV8Context) {
                return await reusable_vm_1.ReusableVMWorkflowCreator.create(workflowBundle, compiledOptions.isolateExecutionTimeoutMs, registeredActivityNames, patchActivationCallback);
            }
            return await vm_1.VMWorkflowCreator.create(workflowBundle, compiledOptions.isolateExecutionTimeoutMs, registeredActivityNames, patchActivationCallback);
        }
        else {
    ...
    activityOperator() {
        return (0, rxjs_1.pipe)((0, rxutils_1.closeableGroupBy)(({ base64TaskToken }) => base64TaskToken), (0, operators_1.mergeMap)((group$) => {
            return group$.pipe((0, rxutils_1.mergeMapWithState)(async (activity, { task, base64TaskToken, protobufEncodedTask }) => {
                ...
                switch (variant) {
                    case 'start': {
                        ...
                        try {
                            ...
                            await (0, internal_non_workflow_1.visit)(task, internal_non_workflow_1.walkActivityTask, (0, internal_non_workflow_1.extstoreInboundOptions)(loadedDataConverter.externalStorage));
                            ...
                            const typeInfo = (0, common_1.isActivityFunctionWithOptions)(fn)
                                ? fn.activityDefinitionOptions.typeInfo
                                : undefined;
                            const outputTypeInfo = typeInfo?.outputType;
                            let args;
                            try {
                                args = await (0, internal_non_workflow_1.decodeArrayFromPayloads)(loadedDataConverter, task.start?.input, context, typeInfo?.inputTypes);
                            }
                            catch (err) {
                            ...
                            activity = new activity_1.Activity(info, fn, loadedDataConverter, context, outputTypeInfo, (details) => this.activityHeartbeatSubject.next({
                                type: 'heartbeat',
                                info: info,
                                taskToken,
                                base64TaskToken,
                            ...
                            output = {
                                ...
                                result: {
                                    failed: {
                                        failure: await (0, internal_non_workflow_1.encodeErrorToFailure)(loadedDataConverter, error, context),
                                    },
                                },
                            };
                            break;
            ...
            }), (0, operators_1.filter)((result) => result !== undefined), (0, operators_1.mergeMap)(async ({ taskToken, result, info }) => {
                ...
                if (externalStorage) {
                    ...
                    try {
                        await (0, internal_non_workflow_1.visit)(completion, internal_non_workflow_1.walkActivityTaskCompletion, (0, internal_non_workflow_1.extstoreStoreOptions)(externalStorage, { initialTarget }));
                    }
                    catch (e) {
                        const error = (0, common_1.ensureApplicationFailure)(e);
                        ...
                        this.logger.error(`Error while offloading ActivityTask result to external storage: ${(0, type_helpers_1.errorMessage)(error)}`, {
                            ...(info ? (0, activity_1.activityLogAttributes)(info) : { taskToken: formatTaskToken(taskToken) }),
                            error: e,
                        });
                        completion.result = {
                            failed: {
                ...
                return proto_1.coresdk.ActivityTaskCompletion.encodeDelimited(completion).finish();
            }), (0, operators_1.tap)({
                next: () => {
                    group$.close();
                },
    ...
    nexusOperator() {
        return (0, rxjs_1.pipe)((0, operators_1.mergeMap)(async ({ task, base64TaskToken, protobufEncodedTask, }) => {
            ...
            switch (variant) {
                case 'task': {
                    ...
                    try {
                        await (0, internal_non_workflow_1.visit)(task, internal_non_workflow_1.walkNexusTask, (0, internal_non_workflow_1.extstoreInboundOptions)(this.options.loadedDataConverter.externalStorage));
                    }
                    catch (e) {
                        this.logger.error(`Error while retrieving Nexus task payloads from external storage: ${(0, type_helpers_1.errorMessage)(e)}`, { taskToken: base64TaskToken, error: e });
                        return {
                            taskToken: task.task.taskToken,
                            failure: await (0, conversions_1.handlerErrorToProto)(this.options.loadedDataConverter, new nexus.HandlerError('INTERNAL', (0, type_helpers_1.errorMessage)(e), { cause: e, retryableOverride: true })),
                        };
                    }
                    const requestDeadline = task.requestDeadline != null ? (0, time_1.tsToDate)(task.requestDeadline) : undefined;
        ...
        }), (0, operators_1.filter)((result) => result !== undefined), (0, operators_1.mergeMap)(async (result) => {
            const { externalStorage } = this.options.loadedDataConverter;
            let completion = result;
            if (externalStorage) {
                try {
                    await (0, internal_non_workflow_1.visit)(completion, internal_non_workflow_1.walkNexusTaskCompletion, (0, internal_non_workflow_1.extstoreStoreOptions)(externalStorage));
                }
                catch (e) {
                    this.logger.error(`Error while offloading Nexus task result to external storage: ${(0, type_helpers_1.errorMessage)(e)}`, {
            ...
            return proto_1.coresdk.nexus.NexusTaskCompletion.encodeDelimited(completion).finish();
        }));
    }
    async handleNexusRunTask(task, base64TaskToken, protobufEncodedTask, requestDeadline) {
        ...
        try {
            ...
            nexusHandler = new nexus_1.NexusHandler(taskToken, this.options.namespace, this.options.taskQueue, (0, nexus_1.constructNexusOperationContext)(task.request, abortController.signal, requestDeadline), this.client, // Must be defined if we are handling Nexus tasks.
            abortController, this.options.nexusServiceHandlers, // Must be defined if we are handling Nexus tasks.
            ...
            return {
                taskToken,
                failure: await (0, conversions_1.handlerErrorToProto)(this.options.loadedDataConverter, handlerError),
            };
        }
    }
    /**
    ...
    async handleActivation(workflow, { activation, synthetic }) {
        try {
            ...
            if (workflowCodecRunner == null) {
                ...
                workflowCodecRunner = new workflow_codec_runner_1.WorkflowCodecRunner(this.options.loadedDataConverter.payloadCodecs, {
                    type: 'workflow',
                    namespace: this.options.namespace,
                    workflowId,
                });
            ...
            const downloadMetrics = externalStorage ? new internal_non_workflow_1.ExternalStorageMetricsAccumulator() : undefined;
            let uploadMetrics;
            await (0, internal_non_workflow_1.visit)(activation, internal_non_workflow_1.walkWorkflowActivation, (0, internal_non_workflow_1.extstoreInboundOptions)(externalStorage, { metrics: downloadMetrics }));
            ...
            try {
                ...
                if (externalStorage && !this.isReplayWorker) {
                    ...
                    uploadMetrics = new internal_non_workflow_1.ExternalStorageMetricsAccumulator();
                    await (0, internal_non_workflow_1.visit)(encodedCompletion, internal_non_workflow_1.walkWorkflowActivationCompletion, (0, internal_non_workflow_1.extstoreStoreOptions)(externalStorage, {
                        initialTarget: {
                            kind: 'workflow',
                            namespace,
                ...
                const completion = proto_1.coresdk.workflow_completion.WorkflowActivationCompletion.encodeDelimited(encodedCompletion).finish();
    ...
    async createWorkflow(activation, initWorkflowJob, workflowCodecRunner) {
        ...
        const { workflowId, randomnessSeed, workflowType, parentWorkflowInfo, rootWorkflow, workflowExecutionTimeout, workflowRunTimeout, workflowTaskTimeout, continuedFromExecutionRunId, firstExecutionRunId, originalExecutionRunId, retryPolicy, attempt, cronSchedule, workflowExecutionExpirationTime, cronScheduleToScheduleInterval, priority, } = initWorkflowJob;
        ...
        const workflowInfo = {
            ...
            suggestedContinueAsNewReasons: (0, continue_as_new_1.suggestContinueAsNewReasonsFromProto)(activation.suggestContinueAsNewReasons),
            currentBuildId: activation.deploymentVersionForCurrentTask?.buildId ?? '',
            currentDeploymentVersion: (0, common_1.convertDeploymentVersion)(activation.deploymentVersionForCurrentTask),
            unsafe: {
                now: () => Date.now(), // re-set in initRuntime
                isReplaying: activation.isReplaying,
                isReplayingHistoryEvents: activation.isReplaying,
                random: (0, random_helpers_1.createUnsafeRandomSource)(Math.random),
            },
     ...
     */
    async processSinkCalls(externalCalls, logAttributes) {
    ...
    activityHeartbeat$() {
        ...
        { processing: false, closed: true }), (0, operators_1.filter)((out) => out != null), (0, operators_1.tap)((out) => {
            ...
            try {
                try {
                    payload = await (0, internal_non_workflow_1.encodeToPayload)(this.options.loadedDataConverter, details, context);
                }
                catch (error) {
                ...
                const heartbeat = { taskToken, details: [payload] };
                const { externalStorage } = this.options.loadedDataConverter;
                if (externalStorage) {
                    await (0, internal_non_workflow_1.visit)(heartbeat, internal_non_workflow_1.walkActivityHeartbeat, (0, internal_non_workflow_1.extstoreStoreOptions)(externalStorage, { initialTarget: activityStorageTarget(info) }));
                }
                const arr = proto_1.coresdk.ActivityHeartbeat.encodeDelimited(heartbeat).finish();
    ...
    async runInternal() {
        ...
        try {
            ...
            try {
                ...
                if (this._connection) {
                    (0, connection_1.extractReferenceHolders)(this._connection).delete(this.nativeWorker);
                    // Only close if this worker is the creator of the connection
                    if (this._connection instanceof connection_1.InternalNativeConnection) {
 ...
 */
function activityStorageTarget(info) {
 ...
 */
async function extractActivityInfo({ task, dataConverter, taskQueue, activityNamespace, context, }) {
    ...
    try {
        heartbeatDetails = await (0, internal_non_workflow_1.decodeFromPayloadsAtIndex)(dataConverter, 0, start.heartbeatDetails, context);
    }
    catch (e) {
...
function workflowCommandStoreTarget(namespace, info) {
    return (message, typeName, context) => {
        switch (typeName) {
            ...
            case 'coresdk.workflow_commands.CompleteWorkflowExecution': {
                const { parent } = info;
                if (parent == null || info.continuedFromExecutionRunId != null)
                    return context;
                return {
                    kind: 'workflow',
...
function activitySerializationContextFromInfo(info) {
    return {
        type: 'activity',
...
function activitySerializationContextFromTaskStart(start, activityNamespace) {
    return {
        type: 'activity',
 ...
 */
function resolveFailureExceptionTypeNames(workflowType, workflowFailureErrorTypes) {
    if (!workflowFailureErrorTypes)
        return undefined;
    const typeNames = new Set();
    for (const name of workflowFailureErrorTypes['*'] ?? [])
        typeNames.add(name);
    for (const name of workflowFailureErrorTypes[workflowType] ?? [])
        typeNames.add(name);
