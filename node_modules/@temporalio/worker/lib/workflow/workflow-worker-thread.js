...
let workflowCreator;
let workflowGetter;
function requestPatchActivation(workflowInfo, patchId) {
    const resultBuffer = new SharedArrayBuffer(patch_activation_callback_1.PATCH_ACTIVATION_CALLBACK_BUFFER_SIZE);
    const request = {
        type: 'patch-activation-callback',
        workflowInfo: (0, patch_activation_callback_1.makePatchActivationWorkflowInfoSnapshot)(workflowInfo),
        patchId,
        resultBuffer,
    };
    parentPort.postMessage(request);
    return (0, patch_activation_callback_1.waitForPatchActivationCallbackResult)(resultBuffer);
}
/**
    ...
    switch (input.type) {
        case 'init':
            if (input.reuseV8Context) {
                workflowCreator = await reusable_vm_1.ReusableVMWorkflowCreator.create(input.workflowBundle, input.isolateExecutionTimeoutMs, input.registeredActivityNames, input.hasPatchActivationCallback ? requestPatchActivation : undefined);
                workflowGetter = (runId) => reusable_vm_1.ReusableVMWorkflowCreator.workflowByRunId.get(runId);
            }
            else {
                workflowCreator = await vm_1.VMWorkflowCreator.create(input.workflowBundle, input.isolateExecutionTimeoutMs, input.registeredActivityNames, input.hasPatchActivationCallback ? requestPatchActivation : undefined);
        ...
        case 'activate-workflow': {
            ...
            const maybeEncodedCompletion = bun_1.isBunPre1_4
        ...
        case 'extract-sink-calls': {
            ...
            calls.forEach((call) => {
                ...
                delete call.workflowInfo.unsafe.random;
