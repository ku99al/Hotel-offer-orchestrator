...
require("@temporalio/workflow/lib/global-attributes"); // eslint-disable-line import/no-unassigned-import
...
function injectGlobals(context) {
    const sandboxGlobalThis = context;
    const globals = {
        URL: node_url_1.URL,
    ...
    function makeConsoleFn(level) {
        return function (...args) {
            if (sandboxGlobalThis.__TEMPORAL_ACTIVATOR__ === undefined) {
                ...
                if (info.unsafe.isReplayingHistoryEvents)
                    return;
    ...
    node_vm_1.default.runInContext(`if (typeof Symbol.dispose !== 'symbol') Object.defineProperty(Symbol, 'dispose', { value: Symbol.for('nodejs.dispose'), writable: false, enumerable: false, configurable: false });
     if (typeof Symbol.asyncDispose !== 'symbol') Object.defineProperty(Symbol, 'asyncDispose', { value: Symbol.for('nodejs.asyncDispose'), writable: false, enumerable: false, configurable: false });`, context);
}
/**
...
class BaseVMWorkflow {
    ...
    async activate(activation) {
        try {
            ...
            if (activation.lastSdkVersion)
                this.activator.sdkVersion = activation.lastSdkVersion;
    ...
    async activateQueries(activation) {
        this.activator.mutateWorkflowInfo((info) => ({
            ...info,
            unsafe: {
                ...info.unsafe,
     ...
     */
    async tryUnblockConditionsAndMicrotasksWithManualFlush() {
        for (;;) {
            await new Promise(setImmediate);
            const numUnblocked = this.workflowModule.tryUnblockConditions();
            if (numUnblocked === 0)
