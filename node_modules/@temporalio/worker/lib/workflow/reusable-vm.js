...
const preloadModulesScript = new node_vm_1.default.Script(`__TEMPORAL__.preloadModules?.()`);
function generateCallIntoScopeScript() {
    return `{
 ...
 */
function generateBunNumericGlobalPropertiesWorkaroundScript() {
    return `{
...
class ReusableVMWorkflowCreator {
    ...
    registeredActivityNames, patchActivationCallback) {
        ...
        if (bun_1.isBun) {
            node_vm_1.default.runInContext(generateBunNumericGlobalPropertiesWorkaroundScript(), this._context, {
                timeout: isolateExecutionTimeoutMs,
                displayErrors: true,
            });
        }
        node_vm_1.default.runInContext(generateCallIntoScopeScript(), this._context, {
            timeout: isolateExecutionTimeoutMs,
            displayErrors: true,
        });
    ...
    async createWorkflow(options) {
        ...
        workflowModule.initRuntime({
            ...
            stackTracesEnabled: vm_shared_1.globalHandlers.promiseHookInstalled,
     ...
     */
    static async create(workflowBundle, isolateExecutionTimeoutMs, registeredActivityNames, patchActivationCallback) {
        ...
        if (!workflowBundle.code.includes('globalThis.__webpack_module_cache__')) {
            throw new Error(`The provided Workflow Bundle is not compatible with the Reusable V8 Context executor: it does ` +
        ...
        return new this(script, workflowBundle, isolateExecutionTimeoutMs, registeredActivityNames, patchActivationCallback);
    }
    /**
