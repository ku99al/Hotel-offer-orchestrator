...
exports.InjectWorkflowModuleCacheGlobalPlugin = void 0;
exports.assertWorkflowModuleCacheGlobalApplied = assertWorkflowModuleCacheGlobalApplied;
...
class InjectWorkflowModuleCacheGlobalPlugin {
    ...
    redirectModuleCache(source) {
        ...
        if (code.indexOf(MARKER_COMMENT, markerIndex + MARKER_COMMENT.length) !== -1) {
            throw new webpack_1.WebpackError(`Failed to patch the Workflow bundle: expected exactly one module-cache marker, but found more than one. ` +
        ...
        if (declaration === null) {
            throw new webpack_1.WebpackError(`Failed to patch the Workflow bundle: found the module-cache marker but no preceding ` +
        ...
        const replaced = new webpack_1.sources.ReplaceSource(source);
...
exports.InjectWorkflowModuleCacheGlobalPlugin = InjectWorkflowModuleCacheGlobalPlugin;
/**
 ...
 */
function assertWorkflowModuleCacheGlobalApplied(code) {
    ...
    if (!code.includes(MODULE_CACHE_GLOBAL)) {
        throw new Error(`Failed to patch the Workflow bundle: the module cache was not redirected to ` +
    ...
    if (code.includes(MODULE_CACHE_MARKER)) {
        throw new Error(`Failed to patch the Workflow bundle: the internal module-cache marker was ` +
...
//# sourceMappingURL=patch-module-cache.js.map