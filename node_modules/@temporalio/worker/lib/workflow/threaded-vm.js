...
const patch_activation_callback_1 = require("./patch-activation-callback");
// https://nodejs.org/api/worker_threads.html#event-exit
// Bun pre 1.4 exits with code 0 instead of 1
exports.TERMINATED_EXIT_CODE = bun_1.isBunPre1_4 ? 0 : 1;
/**
...
class WorkerThreadClient {
    ...
    exitError;
    constructor(workerThread, logger, patchActivationCallback) {
    ...
    handlePatchActivationCallback(request) {
        try {
            if (this.patchActivationCallback === undefined) {
                throw new workflow_1.IllegalStateError('Received patch activation callback request without a configured callback');
            }
            const result = (0, patch_activation_callback_1.invokePatchActivationCallbackWithSnapshot)(this.patchActivationCallback, request.workflowInfo, request.patchId);
            (0, patch_activation_callback_1.writePatchActivationCallbackResult)(request.resultBuffer, result);
        }
        catch (err) {
            (0, patch_activation_callback_1.writePatchActivationCallbackError)(request.resultBuffer, err);
            ...
            (0, patch_activation_callback_1.completePatchActivationCallback)(request.resultBuffer);
        }
    }
    /**
    ...
    async destroy() {
        ...
        const exitCode = await (bun_1.isBunPre1_4 ? this.terminateWithBunWorkaround() : this.workerThread.terminate());
        if (exitCode !== null && exitCode !== exports.TERMINATED_EXIT_CODE) {
     ...
     */
    async terminateWithBunWorkaround() {
        const pollIntervalMs = 100;
        const terminatePromise = this.workerThread.terminate();
        let result = null;
        while (!this.workerExited) {
            result = await Promise.race([terminatePromise, (0, promises_1.setTimeout)(pollIntervalMs, null)]);
        }
        return result;
    }
...
class ThreadedVMWorkflowCreator {
     ...
     */
    static async create({ threadPoolSize, workflowBundle, isolateExecutionTimeoutMs, reuseV8Context, registeredActivityNames, logger, patchActivationCallback, }) {
        const workerThreadClients = Array(threadPoolSize)
            .fill(0)
            .map(() => new WorkerThreadClient(new node_worker_threads_1.Worker(require.resolve('./workflow-worker-thread')), logger, patchActivationCallback));
...
class VMWorkflowThreadProxy {
    ...
    static async create(workerThreadClient, options) {
        ...
        delete options.info.unsafe.random;
    ...
    async getAndResetSinkCalls() {
        ...
        output.calls.forEach((call) => {
            ...
            call.workflowInfo.unsafe.random = (0, random_helpers_1.createUnsafeRandomSource)(Math.random);
        });
        return output.calls;
    }
    /**
    ...
    async activate(activation) {
        const output = await this.workerThreadClient.send({
            ...
            activation: bun_1.isBunPre1_4 ? proto_1.coresdk.workflow_activation.WorkflowActivation.encode(activation).finish() : activation,
            runId: this.runId,
        });
        ...
        if (output.completion instanceof Uint8Array) {
            return proto_1.coresdk.workflow_completion.WorkflowActivationCompletion.decode(output.completion);
        }
