...
class NexusHandler {
     ...
     */
    interceptors;
    constructor(taskToken, namespace, taskQueue, context, client, abortController, services, dataConverter, workerLogger, workerMetricMeter, interceptors) {
    ...
    async startOperation(ctx, payload) {
        try {
            ...
            const input = await (0, conversions_1.decodePayload)(this.dataConverter, payload, handler.inputType);
            const executeNextHandler = async (interceptorInput) => {
                const result = await this.invokeUserCode('startOperation', handler.start.bind(handler, interceptorInput.ctx, interceptorInput.input));
                return { result };
            };
            const executeWithInterceptors = (0, interceptors_1.composeInterceptors)(this.interceptors.inbound, 'startOperation', executeNextHandler);
            ...
            if (result.isAsync) {
                ...
                return {
                    ...
                    completed: {
                        startOperation: {
                            syncSuccess: {
                                payload: await (0, internal_non_workflow_1.encodeToPayload)(this.dataConverter, result.value, undefined, handler.outputType),
                                links: ctx.outboundLinks.map(nexusLinkToProtoLink),
                            },
                        },
                    },
            ...
            if (err instanceof nexus.OperationError) {
                return {
                    ...
                    completed: {
                        startOperation: {
                            failure: await (0, conversions_1.operationErrorToProto)(this.dataConverter, err),
                        },
                    },
                };
            }
            return {
                taskToken: this.taskToken,
                failure: await (0, conversions_1.handlerErrorToProto)(this.dataConverter, (0, conversions_1.coerceToHandlerError)(err)),
            };
        }
    }
    async cancelOperation(ctx, token) {
        try {
            const handler = this.getOperationHandler(ctx);
            const cancelNextHandler = async (interceptorInput) => {
                await this.invokeUserCode('cancelOperation', handler.cancel.bind(handler, interceptorInput.ctx, interceptorInput.token));
            };
            const cancelWithInterceptors = (0, interceptors_1.composeInterceptors)(this.interceptors.inbound, 'cancelOperation', cancelNextHandler);
    ...
    async execute(task) {
        if (task.request?.startOperation != null) {
            ...
            if (!variant.requestId) {
                throw new common_1.IllegalStateError('Missing requestId in Nexus start operation request');
            }
...
function constructNexusOperationContext(request, abortSignal, requestDeadline) {
    const base = {
        abortSignal,
