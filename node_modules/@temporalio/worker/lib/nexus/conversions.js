...
exports.PAYLOAD_VALIDATION_ERROR_TYPE = void 0;
 ...
 */
exports.PAYLOAD_VALIDATION_ERROR_TYPE = 'PayloadValidationError';
/**
 * Whether `err` is a non-retryable {@link ApplicationFailure} whose type is exactly
 * {@link PAYLOAD_VALIDATION_ERROR_TYPE}.
 */
function isPayloadValidationFailure(err) {
    return err instanceof common_1.ApplicationFailure && err.nonRetryable === true && err.type === exports.PAYLOAD_VALIDATION_ERROR_TYPE;
}
/** Decode Payload Codecs and apply optional TypeInfo while translating invalid Nexus input errors. */
async function decodePayload(dataConverter, payload, typeInfo) {
    let decoded;
    try {
        ...
        if (err instanceof common_1.ApplicationFailure || err instanceof nexus.HandlerError) {
            throw err;
        }
    ...
    try {
        return (0, common_1.fromPayloadWithTypeInfo)(dataConverter.payloadConverter, decoded, undefined, typeInfo);
    }
    catch (err) {
        if (isPayloadValidationFailure(err)) {
            throw new nexus.HandlerError('BAD_REQUEST', `Invalid operation input`, {
...
function coerceToHandlerError(err) {
    ...
    if (err instanceof common_1.WorkflowExecutionAlreadyStartedError || err instanceof client_1.ActivityExecutionAlreadyStartedError) {
