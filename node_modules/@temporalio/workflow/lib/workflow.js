...
exports.defineWorkflowOptions = defineWorkflowOptions;
...
const event_groups_1 = require("./event-groups");
        ...
        if (scope.cancellable) {
            (0, stack_helpers_1.untrackPromise)(scope.cancelRequested.catch((err) => {
                ...
                activator.pushCommand({
                    ...
                    eventGroupMarkers: (0, event_groups_1.eventGroupMarkersToProto)(options?.eventGroups),
                });
                reject(err);
            }));
        }
 ...
 */
function scheduleActivityNextHandler({ options, args, headers, seq, activityType, typeInfo, }) {
    const activator = (0, global_attributes_1.getActivator)();
    ...
    return new Promise((resolve, reject) => {
        ...
        activator.pushCommand({
            scheduleActivity: {
                ...
                activityId,
                activityType,
                arguments: (0, common_1.toPayloadsWithContext)(activator.payloadConverter, context, args, typeInfo?.inputTypes),
                ...
                headers,
                cancellationType: (0, activities_1.encodeActivityCancellationType)(options.cancellationType),
 ...
 */
async function scheduleLocalActivityNextHandler({ options, args, headers, seq, activityType, attempt, originalScheduleTime, typeInfo, }) {
    const activator = (0, global_attributes_1.getActivator)();
    ...
    // the activity.
    if (!activator.info.unsafe.isReplaying &&
 ...
 * @hidden
 */
function scheduleActivity(activityType, args, options, typeInfo) {
 ...
 * @hidden
 */
async function scheduleLocalActivity(activityType, args, options, typeInfo) {
    ...
    const startPromise = new Promise((resolve, reject) => {
        ...
        activator.pushCommand({
            startChildWorkflowExecution: {
                ...
                workflowId,
                workflowType,
                input: (0, common_1.toPayloadsWithContext)(activator.payloadConverter, context, options.args, options.typeInfo?.inputTypes),
                retryPolicy: options.retry ? (0, common_1.compileRetryPolicy)(options.retry) : undefined,
                taskQueue: options.taskQueue || activator.info.taskQueue,
...
function signalWorkflowNextHandler({ seq, signalName, args, typeInfo, target, headers }) {
    const activator = (0, global_attributes_1.getActivator)();
    ...
    return new Promise((resolve, reject) => {
        ...
        activator.pushCommand({
            signalExternalWorkflowExecution: {
                seq,
                args: (0, common_1.toPayloadsWithContext)(activator.payloadConverter, context, args, typeInfo?.inputTypes),
                headers,
                signalName,
            ...
            eventGroupMarkers: (0, event_groups_1.eventGroupMarkersToProto)(undefined),
        });
    ...
    return new Proxy({}, {
        get(_, activityType) {
            ...
            function activityProxyFunction(...args) {
                ...
                return scheduleActivity(activityName, args, activityOptions, activityTypeInfo?.[activityName]);
            }
            activityProxyFunction.executeWithOptions = function (overrideOptions, args) {
                const { activityTypeInfo, ...activityOptions } = options;
                return scheduleActivity(activityName, args, (0, internal_workflow_1.deepMerge)(activityOptions, overrideOptions), activityTypeInfo?.[activityName]);
            };
    ...
    return new Proxy({}, {
        get(_, activityType) {
            ...
            function localActivityProxyFunction(...args) {
                const { activityTypeInfo, ...activityOptions } = options;
                return scheduleLocalActivity(activityName, args, activityOptions, activityTypeInfo?.[activityName]);
            }
            localActivityProxyFunction.executeWithOptions = function (overrideOptions, args) {
                const { activityTypeInfo, ...activityOptions } = options;
                return scheduleLocalActivity(activityName, args, (0, internal_workflow_1.deepMerge)(activityOptions, overrideOptions), activityTypeInfo?.[activityName]);
            };
...
function getExternalWorkflowHandle(workflowId, runId) {
    ...
    const signal = (signalName, args, typeInfo) => {
    ...
    return {
        ...
        signalWithOptions(signalName, options) {
...
async function startChild(workflowTypeOrFunc, options) {
    ...
    const { type: workflowType, typeInfo } = (0, common_1.extractWorkflowTypeAndConfig)(workflowTypeOrFunc, optionsWithDefaults.typeInfo);
    ...
    return {
        ...
        async signalWithOptions(signalName, options) {
            return signal(signalName, options.args ?? [], options.typeInfo);
        },
    };
}
...
function makeContinueAsNewFunc(options) {
    ...
    return (...args) => {
        ...
        const fn = (0, interceptor_composition_1.composeInterceptors)(activator.interceptors.outbound, 'continueAsNew', async (input) => {
            ...
            const typeInfo = options.typeInfo ?? (options.workflowType === info.workflowType ? activator.typeInfo : undefined);
            ...
            }, (0, event_groups_1.eventGroupMarkersToProto)(options.eventGroups));
        });
        return fn({
            args,
            headers: {},
 ...
 */
function defineUpdate(name, options = {}) {
    return {
        type: 'update',
        name,
        ...options,
 ...
 */
function defineSignal(name, options = {}) {
    return {
        type: 'signal',
        name,
        ...options,
...
function setHandler(def, handler, options) {
    ...
    if (def.type === 'update') {
        ...
        if (typeof handler === 'function') {
            activator.queryHandlers.set(def.name, { handler: handler, description, typeInfo: def.typeInfo });
        }
        else if (handler == null) {
 ...
 * @experimental
 */
function defineWorkflowOptions(fn, config) {
